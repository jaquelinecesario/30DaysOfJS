...
function hasManySkills(obj) {
  const usersNames = Object.keys(obj).sort();
  const skillsArray = [
    obj.Alex.skills,
    obj.Asab.skills,
    obj.Brook.skills,
  ...
  for (let i = 0; i < skillsArray.length; i++) {
    for (let c = 0; c < skillsArray.length; c++) {
      if (i === c) {
        continue;
      } else if (skillsArray[c] > skillsArray[i] && skillsArray[c] > auxArray) {
        manySkills = usersNames[c];
        auxArray[0] = skillsArray[c];
      }
    }
  }
  return `${manySkills} has many skills in the users object.`;
}
console.log(hasManySkills(users));

function moreFiftyPoints(obj) {
  const isLoggedInArray = [
    obj.Alex.isLoggedIn,
    obj.Asab.isLoggedIn,
    obj.Brook.isLoggedIn,
    obj.Daniel.isLoggedIn,
    obj.John.isLoggedIn,
    obj.Paul.isLoggedIn,
    obj.Thomas.isLoggedIn,
  ];
  const pointsArray = [
    obj.Alex.points,
    obj.Asab.points,
  ...
  for (let i = 0; i < isLoggedInArray.length; i++) {
    if (isLoggedInArray[i] === true && pointsArray[i] >= 50) {
      sum++;
    }
  }

  return `${sum} users are logged in and have greater than equal to points`;
}
console.log(moreFiftyPoints(users));

function mernStackDeveloper(obj) {
  ...
  for (let i = 0; i < usersNames.length; i++) {
    ...
    for (let c = 0; c < mernStack.length; c++) {
      if (skillsArray[i].includes(mernStack[c])) {
        mern.push(mernStack[c]);
      }
    }
    if (mern.length === 4) {
      auxArray.push(usersNames[i]);
      sum++;
    }
  }

  return `There are ${sum} - ${auxArray.join(", ")} - MERN stack developers.`;
}
console.log(mernStackDeveloper(users));
...
const country = countries[25];
let result = `
Country: ${country.name} \n
Capital: ${country.capital} \n
Poputalion: ${country.population} \n
